...
\u7248\u672C: \`${r.version}\``),new K.Hover(a,n)}};var v=l(require("vscode")),Y=l(require("path")),ke=l(require("os")),R=l(require("fs"));var Ae=new Map;async function z(o,e){let t=o.id?o:await L(o.id).catch(()=>o),s=[];try{s=await N(t.id)}catch{v.window.showErrorMessage(`\u83B7\u53D6\u7B97\u6CD5\u6587\u4EF6\u5931\u8D25: ${t.id}`);return}if(s.length===0){v.window.showWarningMessage("\u8BE5\u7B97\u6CD5\u6682\u65E0\u6587\u4EF6");return}let n=Y.join(ke.tmpdir(),"algolib-edit",t.id.replace(/[/\\:]/g,"_"));R.existsSync(n)||R.mkdirSync(n,{recursive:!0});for(let a of s){let c=Y.join(n,a.filename);R.writeFileSync(c,a.content,"utf8");let w=c;Ae.set(w,{algorithmId:t.id,filename:a.filename,tmpPath:c,savedVersion:a.content})}let i=s.find(a=>a.filename.endsWith(".py"))??s[0],r=v.Uri.file(Y.join(n,i.filename));await v.window.showTextDocument(r,{preview:!1})}function Se(o){o.subscriptions.push(v.workspace.onWillSaveTextDocument(async e=>{let t=e.document,s=t.uri.fsPath,n=Ae.get(s);if(!n)return;let i=t.getText();i!==n.savedVersion&&e.waitUntil((async()=>{try{await q(n.algorithmId,n.filename,i),n.savedVersion=i,v.window.setStatusBarMessage(`$(check) AlgoLib: ${n.filename} \u5DF2\u4FDD\u5B58`,3e3)}catch(r){v.window.showErrorMessage(`\u4FDD\u5B58\u5931\u8D25: ${String(r)}`)}return[]})())}))}var d=l(require("vscode"));var E=l(require("vscode"));var C=l(require("vscode")),I=class o{static getInstance(){return o.instance||(o.instance=new o),o.instance}show(e,t){this.panel||(this.panel=C.window.createWebviewPanel("algolib.output","AlgoLib \u8F93\u51FA",C.ViewColumn.Beside,{enableScripts:!0,retainContextWhenHidden:!0,localResourceRoots:[C.Uri.joinPath(e.extensionUri,"media")]}),this.panel.onDidDispose(()=>{this.panel=void 0})),this.panel.webview.html=this.getHtml(e,t),this.panel.reveal(C.ViewColumn.Beside,!0)}showStream(e){this.show(e)}appendOutput(e,t){this.panel?.webview.postMessage({command:"append",kind:e,text:t})}showResult(e){this.panel?.webview.postMessage({command:"result",data:e})}clear(){this.panel?.webview.postMessage({command:"clear"})}getHtml(e,t){let s=this.panel.webview.cspSource,n=this.panel.webview.asWebviewUri(C.Uri.joinPath(e.extensionUri,"media","shared.css")),i=this.panel.webview.asWebviewUri(C.Uri.joinPath(e.extensionUri,"media","echarts.min.js"));return`<!DOCTYPE html>
<html lang="zh">
<head>
<meta charset="UTF-8"/>
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src ${s} 'unsafe-inline'; script-src ${s} 'unsafe-inline'; img-src ${s} data: blob:; font-src ${s};" />
