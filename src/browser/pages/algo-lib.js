;(function () {
  ...
  async function publishAsComponent(templateId) {
    try {
      ...
      const created = await submitJson(`${apiBase}/templates/${templateId}/publish`, "POST", payload)
      if (created?.id) {
        state.selectedAlgorithmId = created.id
      }
      state.algorithmScope = "mine"
      showToast("✅ 已发布为组件草稿，请前往算法组件页面继续编辑", 3000)
      setTimeout(() => {
  ...
  function bindPackageEditorButtons(root, mode) {
    ...
    root.querySelector("[data-pkg-action='add-file']")?.addEventListener("click", () => addPkgFile())
  ...
  const renderGalleryEditorShell = (item, content) => {
    ...
    editor.innerHTML = `
      <div class="pkg-topbar">
        ...
        <label class="pkg-namespace-field">命名空间
          <input id="galleryNamespaceInput" class="pkg-namespace-input" value="${escapeHtml(item.namespace || "")}" />
        </label>
        <div class="pkg-topbar-actions">
          <button class="pkg-btn" data-pkg-action="close" type="button">← 返回列表</button>
          <button class="pkg-btn" data-pkg-action="save-file" type="button">保存</button>
      ...
      <div class="pkg-body">
        <div class="pkg-editor-area">
          <div class="pkg-tab-bar">
            <div class="pkg-tab active"><span>${escapeHtml(item.funcName || item.id || "source.py")}</span><span id="galleryModifiedDot"></span></div>
          </div>
          <div class="monaco-container" id="monacoContainer"></div>
          <textarea class="pkg-fallback-editor hidden" id="pkgFallbackEditor" spellcheck="false"></textarea>
        </div>
  ...
  async function openGalleryAlgorithmEditor(algorithmId) {
    ...
    try {
      ...
      _monacoEditor = null
      _monacoModels = {}
      renderGalleryEditorShell(state.currentGalleryAlgorithm, payload.content || "")
      await initGallerySourceEditor(payload.content || "", inferLanguageFromFileName(`${item.funcName || "source"}.py`))
      initVResizeDrag()
    } catch (error) {
      showToast(error.message || String(error), "error")
    }
  ...
  async function initGallerySourceEditor(content, language = "python") {
    const fallback = document.getElementById("pkgFallbackEditor")
    const monacoHost = document.getElementById("monacoContainer")
    if (fallback) {
      fallback.value = content || ""
  ...
  async function saveGalleryAlgorithmSource() {
    ...
    try {
      ...
      if (payload.algorithm) {
        ...
        if (nsInput) nsInput.value = payload.algorithm.namespace || nsInput.value
      }
      const dot = document.getElementById("galleryModifiedDot")
  ...
  const renderTemplates = () => {
    ...
    $("#template-multifile-new").classList.toggle("hidden", !isAdmin())
    ...
    $("#template-multifile").classList.toggle("hidden", !hasTemplateContext || !isAdmin())
    $("#template-multifile").textContent = template?.packageId ? "编辑多文件包" : "转为多文件"
  ...
  const renderAlgorithms = () => {
    ...
    $("#algorithm-new").classList.toggle("hidden", state.algorithmScope !== "mine")
    $("#algorithm-multifile-new").classList.toggle("hidden", state.algorithmScope !== "mine")
    $("#algorithm-folder-new").classList.toggle("hidden", state.algorithmScope !== "mine")
    ...
    $("#algorithm-multifile").classList.toggle("hidden", !hasAlgorithmContext || algorithmReadOnly)
    $("#algorithm-multifile").textContent = algorithm?.packageId ? "编辑多文件包" : "转为多文件"
...
utils.py</textarea></label>
  ...
  const renderComponentGallery = () => {
    ...
    const filtered = state.galleryAlgorithms.filter((item) => {
      ...
      if (q) {
        const hay = `${item.zhName || ""} ${item.callPrefix || ""} ${item.zhDescription || ""} ${(item.zhTags || []).join(" ")}`.toLowerCase()
      ...
      items.forEach((item, idx) => {
        ...
        html += `
            <div class="ag-card"
              ...
              data-call-prefix="${escapeHtml(item.callPrefix || "")}"
              data-call-snippet="${escapeHtml(item.callSnippet || item.callPrefix || "")}"
              tabindex="0" role="button">
              <div class="ag-card-top">
                <div class="ag-card-icon">${icon}</div>
                <div class="ag-card-head">
                  <div class="ag-card-name">${escapeHtml(item.zhName || item.callPrefix || "")}</div>
              ...
              <div class="ag-card-foot">
                <div class="ag-card-call"><code>${escapeHtml(item.callPrefix || "")}</code></div>
                <div class="ag-card-actions">
                  <button class="ag-ib" title="复制调用" data-action="copy-call" data-call="${escapeHtml(item.callPrefix || "")}">
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
                  </button>
                  <button class="ag-ib ag-ib-go" title="插入调用" data-action="insert-call" data-call="${escapeHtml(item.callSnippet || item.callPrefix || "")}">
  ...
  window.patchNamespace = async function (algorithmId, newNamespace) {
    ...
    try {
      ...
      const nsOnly = parts[1] || val
      const inputEl = document.getElementById("nsEditInput")
      const wrapper = inputEl?.closest(".pkg-namespace-field") || inputEl?.parentElement
      if (wrapper) {
        wrapper.innerHTML = `<input id="galleryNamespaceInput" class="pkg-namespace-input" value="${escapeHtml(nsOnly)}" />`
      }
