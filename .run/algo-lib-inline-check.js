    ...
    function renderCard(item, page) {
      ...
      if (effectivePage === "components") {
        ...
        if (canSubmitAlgorithm(item) || (isOwner && status === "published" && item.hasReviewDraft)) btns.push(`<button class="warning" onclick="window.openSubmitModal('${esc(id)}')">${status === "rejected" ? "重新提交" : "提交审核"}</button>`);
        if (canManage && status === "reviewing") btns.push(`<button onclick="window.withdrawReview('${esc(id)}')">撤回</button>`);
        if (isAdmin && status === "reviewing") btns.push(`<button onclick="window.approveReview('${esc(id)}')">审核通过</button>`);
        if (isAdmin && status === "approved") btns.push(`<button class="success" onclick="window.publishComponent('${esc(id)}')">正式发布</button>`);
        if (isAdmin || (canManage && status !== "published")) btns.push(`<button class="danger" onclick="window.deleteAlgorithm('${esc(id)}')">删除</button>`);
        ...
        if (isAdmin && status === "reviewing") {
          btns.push(`<button onclick="window.approveSnippetReview('${esc(id)}')">审核通过</button>`);
          btns.push(`<button class="danger" onclick="window.rejectSnippetReview('${esc(id)}')">驳回</button>`);
        }
        if (isAdmin && status === "approved") btns.push(`<button class="success" onclick="window.publishSnippet('${esc(id)}')">正式发布</button>`);
      ...
      return `
        <article class="algo-card ${kind} ${state.highlightId === id ? "highlight" : ""}" data-id="${esc(id)}">
          ...
          <span class="tag ${privacyClass}" style="position:absolute;right:14px;top:${reviewStatusVisible ? "42px" : "14px"}">${privacyText}</span>
...
print(result)`;
    ...
    async function editSnippet(id, fork = false) {
      ...
      if (fork || publishedReadOnly) {
        const baseName = String(snippet.name || "snippet").replace(/_copy(?:_\d+)?$/, "");
        snippet = {
          ...snippet,
          id: "",
          name: `${baseName}_copy`,
    ...
    async function renderReviewPage() {
      ...
      try {
        ...
        const items = [...(state.data.components || []), ...(state.data.templates || [])].filter(item => {
          ...
          <table class="api-table">
            ...
            <tbody>${items.map(item => {
              ...
              return `<tr>
                ...
                <td><code>${esc(getNs(item, item.moduleKind === "template" || item.type === "template" ? "templates" : "components"))}</code></td>
                ...
                <td>
                  <button onclick="window.openEditorById('${esc(item.id)}','${item.moduleKind === "template" || item.type === "template" ? "templates" : "components"}',true)">查看/测试</button>
